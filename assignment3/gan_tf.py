...
def discriminator_loss(logits_real, logits_fake):
    ...
    targets_real = tf.ones_like(logits_real)
    targets_fake = tf.zeros_like(logits_fake)
    loss = tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(targets_real, logits_real))\
         + tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(targets_fake, logits_fake))
...
def generator_loss(logits_fake):
    ...
    targets_fake = tf.ones_like(logits_fake)
    loss = tf.math.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(targets_fake, logits_fake))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = (tf.math.reduce_mean(tf.math.square(scores_real-1)) \
          + tf.math.reduce_mean(tf.math.square(scores_fake)))  \
          / 2
...
def ls_generator_loss(scores_fake):
    ...
    loss = tf.math.reduce_mean(tf.math.square(scores_fake-1)) / 2
...
def dc_generator(noise_dim=NOISE_DIM):
    ...
    model.add(tf.keras.layers.Conv2DTranspose(64, (4, 4), 2, padding='SAME', activation='relu'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.Conv2DTranspose(1, (4, 4), 2, padding='SAME', activation='tanh'))
    model.add(tf.keras.layers.Flatten())
