...
def tv_loss(img, tv_weight):
    ...
    loss = tv_weight \
            * (tf.math.reduce_sum(tf.math.square(img[:,:,1:,:] - img[:,:,:-1,:])) \
            +  tf.math.reduce_sum(tf.math.square(img[:,:,:,1:] - img[:,:,:,:-1])))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i,l in enumerate(style_layers):
      ...
      loss += style_weights[i]*tf.math.reduce_sum(tf.math.square(gram - style_targets[i]))
...
def content_loss(content_weight, content_current, content_original):
    ...
    _, H_l, W_l, C_l = content_current.shape
    shape = (H_l*W_l, C_l)

    F_l = tf.reshape(content_current,  shape)
    P_l = tf.reshape(content_original, shape)

    loss = content_weight*tf.math.reduce_sum(tf.math.square(F_l - P_l))

    return loss
    # return content_weight * 2 * tf.nn.l2_loss(content_original - content_current)
